...
Dialog.addCheckbox("Make the binary mask", false);
...
Dialog.addNumber("Max size:", 1000);
Dialog.show();
...
for (i = 0; i < n; i++) {
    ...
    roiManager("select", i);
    roiManager("Rename", "Outline-cell"+i);
...
// Make table 
titleNewTab2 = "[Foci count table]"; 
run("New... ", "name="+titleNewTab2+" type=Table"); 
print(titleNewTab2,"\\Headings:Cell-Number \t Foci-count");
...
for (i=0; i<RoiCount; i++) { 
		...
		j = 1;
			for (k=RoiFirstSpot; k<NewRoicount; k++) { 
				roiManager("Select", k);
				roiManager("Rename", "Foci"+j+"_Cell"+i);
				j = j+1;
			}		
}
